...
class HyperNet(nn.Module):
    def __init__(
        ...
        if not no_residual: 
            ...
            self.in_channel_estimator = EqualLinear(self.style_dim, self.in_channel, bias_init=1).to(device)
            self.kernel_estimator = EqualLinear(self.style_dim, self.kernel_size ** 2, bias_init=0).to(device)
        
            # weight and  bias init, which are negligible to the main generator
    ...
    def forward(self, domain_style):
        ...
        if not self.no_residual:
            ...
            res_in_channel = self.in_channel_estimator(domain_style)
            res_kernel = self.kernel_estimator(domain_style)
            
            res_conv = torch.bmm(res_out_channel.unsqueeze(-1), torch.bmm( res_in_channel.unsqueeze(-1), res_kernel.unsqueeze(1)).view(batch, -1).unsqueeze(1)).view(batch, self.out_channel, self.in_channel, self.kernel_size, self.kernel_size)
        else:
            res_conv = 0.
        
        if not self.no_scaling:
            domain_mod = self.domain_modulation(domain_style).view(batch, self.out_channel, 1, 1, 1)
        else:
            domain_mod = 1.
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, noise=None, domain_style=None,alpha=1.0, beta=1.0):
        out = self.conv(input, style, domain_style=domain_style, alpha=alpha, beta=beta)
...
class Generator(nn.Module):
    def __init__(
        ...
        self.c_dim = c_dim
        self.is_domain_modulation = False
        self.embedding = None
        self.lr_mlp = lr_mlp
    ...
    def forward(
        ...
        if self.is_domain_modulation and self.multi_domain and not None in domain_labels:
            ...
            if domain_is_latents:
                ...
                domain_styles = [self.embedding(l) for l in domain_labels]
        else:
            domain_styles = [None]
